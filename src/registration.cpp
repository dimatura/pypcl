...
Eigen::Matrix4f organized_pc_registration(PCXYZ::Ptr xyz_src,
  ...
  organized_projection.determineCorrespondences(*correspondences,
                                                max_dist);  // 0.05
  ...
  te.estimateRigidTransformation(
      *xyz_src, *xyz_tgt, *correspondences_filtered, tf);
  // pcl::registration::TransformationEstimationPointToPlaneWeighted<pcl::PointXYZ,
  // pcl::PointNormal, double> te;
  // te.setWeights(correspondence_weights);
  // te.estimateRigidTransformation(*xyz_src, *cloud_normals_tgt,
  // correspondences_filtered, transform);
  return tf;
}
...
Eigen::Matrix4f icp2(const PCLPC2::Ptr pc2_src,
  ...
  CorrespondenceEstimation<PointNormal, PointNormal, Scalar>::Ptr cens(
      new CorrespondenceEstimation<PointNormal, PointNormal, Scalar>);
  cens->setInputSource(src);
  cens->setInputTarget(tgt);
  // cens->setSourceNormals (src);
  CorrespondenceRejectorOneToOne::Ptr cor_rej_o2o(
      new CorrespondenceRejectorOneToOne);
